...
The penalized objective function $h(x)$ is defined as:

$$
h(x) = f(x) + lr \times (penalty\_eq + penalty\_ineq)
$$

where

$$
penalty\_eq = \sum |equality\_constraints(x)|^p
$$

$$
penalty\_ineq = \sum \max(0, inequality\_constraints(x))^p
$$
